...
- **引用付きチャットボット**: アップロード資料に基づいた回答と、その根拠となる資料名の明示。以下の厳密な優先順位に基づきます：
    ...
    4. 製品企画書
    5. 法規リスト
- **インターネット検索 (Improved Web Search)**: 「Tavily AI Search API」を統合。最新トレンドに加え、一般的知識や、社内資料で情報が不十分な場合（スコア連動フォールバック）に自動的にウェブから情報を補完します。
- **Smart Fallback Mechanism**: 内部資料の関連度が低い場合（Score < 5.0）、自動的にWeb検索を実行。社内知識と外部知識をシームレスに統合し、「分かりません」という回答を最小限に抑えます。
